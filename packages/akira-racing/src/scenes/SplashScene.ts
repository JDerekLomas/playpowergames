...
export class SplashScene extends BaseScene {
    ...
    init() {
        ...
        const logo = this.addImage(this.display.width / 2, 55, 'splashscreen_gametitle')
            .setOrigin(0.5, 0);
    ...
    create() {
        ...
        const topic = gameConfigManager.get('topic') || 'grade5_topic1';

        AnalyticsHelper.createInstance('digit_drive', topic);
    }
}