import { BaseScene, ButtonHelper, i18n, VolumeSlider, setSceneBackground, ScoreboardHelper, GameConfigManager, ButtonOverlay, AnalyticsHelper } from '@k8-games/sdk';
import { ASSETS_PATHS, BUTTONS, COMMON_ASSETS, MULTIVERSE_TOPICS, SUCCESS_TEXT_KEYS } from '../config/common';
...
export class ScoreboardScene extends BaseScene {
    ...
    init(data: ScoreboardData) {
        ...
        this.isMultiverse = MULTIVERSE_TOPICS.includes(this.gameConfigManager.get('topic') || '') || false;
        this.topic = this.gameConfigManager.get('topic') || '';
    ...
    create() {
        ...
        this.scoreboardHelper.createTextBlock(this.gameData.rounds, this.gameData.totalRounds, () => {
            ...
            // standalone fact mastery game for grade3_topic2 (show only play again button)
            if (this.isMultiverse && this.topic !== 'grade3_topic2') {
    ...
    private openDoors() {
        animateDoors({
            ...
            onComplete: () => {
                // Do not override user's volume preference here. Respect current audioManager volume.
            }
        });
    }
    ...
    private closeDoors() {
        animateDoors({
            ...
            onComplete: () => {
                this.audioManager.stopBackgroundMusic();
                this.scene.start('Shooter', { reset: true });
                const analyticsHelper = AnalyticsHelper.getInstance();
                if (analyticsHelper) {
