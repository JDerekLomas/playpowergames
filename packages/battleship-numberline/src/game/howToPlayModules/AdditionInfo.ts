...
export interface AdditionInfoData {
    scene: BaseScene;
...
export class AdditionInfo {
    ...
    constructor(data: AdditionInfoData) {
    ...
    private createTutorialContainer(): void {
        ...
        // this.prevButton = ButtonHelper.createIconButton({
        //     scene: this.scene,
        //     imageKeys: {
        ...
        //             this.currentStep--;
        //             this.playAdditionStep(this.currentStep);
        //         }
        //     },
        // });

        // this.nextButton = ButtonHelper.createIconButton({
        //     scene: this.scene,
        //     imageKeys: {
        //         default: buttonKeys.SQ_DEFAULT,
        //         hover: buttonKeys.SQ_HOVER,
        ...
        // this.skipButton = ButtonHelper.createButton({
        //     scene: this.scene,
        //     imageKeys: {
        ...
        // this.tutorialContainer.add([backgroundRect, this.prevButton, this.nextButton, this.skipButton]).setDepth(2).setVisible(false);
    ...
    private playAdditionStep2() {
        ...
        const targetX = startX + (endX - startX) * ((Number(operand1) - Number(this.question.startPoint)) / (Number(this.question.endPoint) - Number(this.question.startPoint)));
    ...
    private playAdditionStep3() {
        this.guideLine?.destroy();

        // Show markers from operand 1 to operand 2
        const leftMarker = this.numberline?.leftMarker;
        const rightMarker = this.numberline?.rightMarker;
        ...
        const operand1 = this.question.csvQuestion?.operand1 ?? '';
        // Calculate the gap between markers
        let gapBetweenMarkers = this.question.markersList.length > 1 
        ? (Number(this.question.endPoint) - Number(this.question.startPoint)) / (this.question.markersList.length - 1)
        : 0;

        if (this.topic === 'place_value_magnitude') gapBetweenMarkers = 500;

        const width = (endX - startX) * (gapBetweenMarkers / (Number(this.question.endPoint) - Number(this.question.startPoint)));

        const operand1Index = this.question.markersList.indexOf(operand1 as never);
        const answerIndex = this.question.markersList.indexOf(this.question.csvQuestion?.answer as never);
        const steps = answerIndex - operand1Index;
        const xOffset = width * operand1Index;

        for (let i = 0; i < steps; i++) { 
            const rect = this.scene.addRectangle(
                startX + xOffset + width * i + width / 2,
        ...
        this.createAdditionStep({
            ...
            elements: [
                ...
                { type: 'text', value: i18n.t('game.tutorial.stepsFromThere') },
            ],
        });
    }

    private playAdditionStep4() {
        this.guideLine.destroy();
        ...
        const targetX = startX + (endX - startX) * ((Number(answer) - Number(this.question.startPoint)) / (Number(this.question.endPoint) - Number(this.question.startPoint)));
    ...
    private async playAdditionStep5() {
        this.guideLine.destroy();
        ...
        const widthRatio = answerIndex * (gapBetweenMarkers / (Number(this.question.endPoint) - Number(this.question.startPoint)));
    ...
    private playAdditionStep(step: number) {
        ...
        switch (step) {
            ...
            case 2:
                ...
                break;
            case 3:
                this.playAdditionStep3();
                break;
            case 4:
                this.playAdditionStep4();
                break;
            case 5:
                this.playAdditionStep5();
                break;
        }
    }
