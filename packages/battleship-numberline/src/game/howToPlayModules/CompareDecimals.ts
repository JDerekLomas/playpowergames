...
export interface CompareDecimalsData {
    scene: BaseScene;
...
export class CompareDecimals {
    ...
    constructor(data: CompareDecimalsData) {
    ...
    private createUpperContainer(): void {
        ...
        // Add the upper text
        let upperTextValue = i18n.t('howToPlay.fractionsStep1');
        if (this.topic === 'understand_and_compare_decimals') {
            upperTextValue = i18n.t('howToPlay.decimalsStep1');
            upperRect.setDisplaySize(this.scene.getScaledValue(Config.UPPER_CONTAINER.RECTANGLE.WIDTH + 100), this.scene.getScaledValue(Config.UPPER_CONTAINER.RECTANGLE.HEIGHT));
        }
        const upperText = this.scene.addText(0, -40, upperTextValue, {
    ...
    // Step 1: Audio introduction with pointing hand
    private playStep1() {
        if (this.currentStep !== 1) return;

        let step1AudioKey = 'step_1_fraction';
        if (this.topic === 'understand_and_compare_decimals') {
            step1AudioKey = 'step_1_decimals';
        }
        const step1 = this.scene.audioManager.playSoundEffect(step1AudioKey);
    ...
    // Step 2: Jump operand times (yellow bars)
    private playStep2() {
        this.pointingHand?.destroy();
        this.upperContainer?.destroy();
        ...
        // Convert decimal to fraction (e.g., 0.2 = 2/10)
        const decimalValue = parseFloat(this.question.csvQuestion?.operand1 ?? '');
        const jumpCount = Math.round(decimalValue * 10); // For 0.2, jump 2 times
        ...
        const width = (endX - startX) / 10; // 10 parts for 0.1, 0.2, ..., 1.0
        const height = 15;
        ...
        this.createCompareDecimalsStep({
            audioKey: this.getAudioKey('step_2'),
            elements: [
                { type: 'text', value: i18n.t('game.tutorial.jump') },
                { type: 'expression', value: jumpCount.toString() },
                { type: 'text', value: i18n.t('game.tutorial.times') },
            ],
        });
    }

    // Step 3: Show answer (guide line)
    private playStep3() {
        ...
        const answerX = startX + width * (decimalValue * 10);
        ...
        this.createCompareDecimalsStep({
            ...
            elements: [
            { type: 'text', value: i18n.t('game.tutorial.goesHere', { number: this.question.csvQuestion?.operand1 }) },
            ],
        });
    }

    // Step 4: Drop bomb (clickable zone)
    private async playStep4() {
        this.guideLine?.destroy();
        ...
        const widthRatio = decimalValue; // 0.2 = 0.2 ratio
    ...
    private playCompareDecimalsStep(step: number) {
    ...
    private createCompareDecimalsStep({
        audioKey,
        elements,
    }: {
        audioKey: string;
        ...
        stepAudio?.on('complete', () => {
            this.currentStep++;
            this.playCompareDecimalsStep(this.currentStep);
        });
    }
