...
export interface FractionAdditionInfoData {
    scene: BaseScene;
...
export class FractionAdditionInfo {
    ...
    constructor(data: FractionAdditionInfoData) {
    ...
    // Step 4: Move operand2 times forward (yellow boxes)
    private playStep4() {
        ...
        for (let i = 0; i < steps; i++) {
            const rect = this.scene.addRectangle(
                startX + width * (operand1Index + i) + width / 2,
                (leftMarker?.line.y ?? 0) / this.scene.display.scale,
                width - 2 * 10,
                height,
                0xFFF600,
        ...
        this.createFractionStep({
            ...
            elements: [
                ...
                { type: 'text', value: steps === 1 ? i18n.t('game.tutorial.stepFromThere') : i18n.t('game.tutorial.stepsFromThere') },
            ],
        });
    }
