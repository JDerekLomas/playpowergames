...
export interface FractionSubtractionInfoData {
    scene: BaseScene;
...
export class FractionSubtractionInfo {
    ...
    constructor(data: FractionSubtractionInfoData) {
    ...
    // Step 2: Split number line into denominator parts (gray boxes)
    private playStep2() {
        ...
        this.createFractionStep({
            ...
            elements: [
                { type: 'text', value: i18n.t('game.tutorial.divideLineInto') },
                { type: 'text', value: parts.toString() },
                { type: 'text', value: i18n.t('game.tutorial.parts') },
            ],
        });
    }

    // Step 3: Locate the first operand (guide line)
    private playStep3() {
        this.tutorialRects.forEach((rect) => rect.destroy());
        this.tutorialRects = [];
        ...
        const endX = (rightMarker?.line.x ?? 0) / this.scene.display.scale;
        const width = (endX - startX) / parts;
        const operand1X = startX + width * operand1Index;
        ...
        this.guideLine = (this.scene as any).createMaskedGuideline(this.scene.getScaledValue(operand1X), startY, this.scene.getScaledValue(420), markers);

        this.createFractionStep({
            audioKey: this.getAudioKey('step_3'),
            elements: [
                { type: 'text', value: i18n.t('game.tutorial.locate') },
                { type: 'expression', value: operand1 },
                { type: 'text', value: i18n.t('game.tutorial.onNumberLine') },
            ],
        });
    }

    // Step 4: Move operand2 times backwards (yellow boxes) - CHANGED FOR SUBTRACTION
    private playStep4() {
        ...
        const steps = operand1Index - answerIndex; // CHANGED: operand1Index - answerIndex for subtraction
        const parts = this.question.markersList.length - 1;
        const leftMarker = this.numberline?.leftMarker;
        const rightMarker = this.numberline?.rightMarker;
        const startX = (leftMarker?.line.x ?? 0) / this.scene.display.scale;
        ...
        for (let i = 0; i < steps; i++) {
            const rect = this.scene.addRectangle(
                startX + xOffset - width * i - width / 2, // FIXED: Use same pattern as SubtractionInfo
                (leftMarker?.line.y ?? 0) / this.scene.display.scale,
                width - 2 * 10,
                height,
                0xFFF600,
        ...
        this.createFractionStep({
            ...
            elements: [
                ...
                { type: 'expression', value: steps.toString() },
                { type: 'text', value: steps === 1 ? i18n.t('game.tutorial.stepBackwards') : i18n.t('game.tutorial.stepsBackwards') }, // CHANGED: backwards text
            ],
        });
    }

    // Step 5: Show result (guide line for answer)
    private playStep5() {
        this.guideLine?.destroy();
        this.tutorialRects.forEach((rect) => rect.destroy());
        this.tutorialRects = [];
    ...
    // Step 6: Tap here (clickable zone)
    private async playStep6() {
        this.guideLine?.destroy();
    ...
    private createFractionStep({
        ...
        positionedElements[0].type !== 'Text' && (stepContainer.x += positionedElements[0].getBounds().width / 2);
