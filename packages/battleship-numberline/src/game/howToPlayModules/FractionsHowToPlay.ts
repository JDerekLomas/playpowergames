...
export interface FractionsHowToPlayData {
    scene: BaseScene;
...
export class FractionsHowToPlay {
    ...
    constructor(data: FractionsHowToPlayData) {
    ...
    private createText(x: number, y: number, text: string, tutorialElements: (Phaser.GameObjects.Text | Phaser.GameObjects.Container)[]) {
        ...
        if (tutorialElements.length > 0) {
            ...
            // Add extra spacing for mixed fractions (though this is less likely for text elements)
            const mixedFractionPattern = /^\s*-?\d+\s+\d+\/\d+\s*$/;
            const extraSpacing = mixedFractionPattern.test(text) ? 25 : 15;
            textElement.setX(bounds.centerX + this.scene.getScaledValue(extraSpacing));
        }
    ...
    private createExpression(x: number, y: number, text: string, tutorialElements: (Phaser.GameObjects.Text | Phaser.GameObjects.Container)[]) {
        ...
        if (tutorialElements.length > 0) {
            const lastElement = tutorialElements[tutorialElements.length - 1];
            const bounds = lastElement.getBounds();
            // Add extra spacing for mixed fractions
            const mixedFractionPattern = /^\s*-?\d+\s+\d+\/\d+\s*$/;
            const extraSpacing = mixedFractionPattern.test(text) ? 25 : 15;
            expressionContainer.setX(bounds.right + this.scene.getScaledValue(extraSpacing));
        }
    ...
    private playFractionStep1() {
        if (this.currentStep !== 1) return;

        const step1 = this.scene.audioManager.playSoundEffect('step_1_fraction');
    ...
    private playFractionStep2() {
        ...
        this.createFractionStep({
            audioKey: this.getAudioKey('step_2'),
            elements: [
                { type: 'text', value: i18n.t('tutorial.step1.1') },
                { type: 'expression', value: this.question.questionPrompt },
                { type: 'text', value: i18n.t('tutorial.step1.2') },
            ],
        });
    }

    private playFractionStep3() {
        ...
        this.createFractionStep({
            ...
            elements: [
                { type: 'text', value: i18n.t('tutorial.step2', { denominator: parts }) },
            ],
        });
    }

    private playFractionStep4() {
        if (this.guideLine) this.guideLine.destroy();

        const answer = this.question.csvQuestion?.answer ?? '';
        const numerator = parseInt(answer.split('/')[0]);

        this.tutorialRects.forEach((rect, index) => {
            rect.fillColor = index < numerator ? 0xFFF600 : 0x303742;
        });

        this.createFractionStep({
            audioKey: this.getAudioKey('step_4'),
            elements: [
                { type: 'text', value: i18n.t('tutorial.step3', { numerator }) },
            ],
        });
    }

    private playFractionStep5() {
        const leftMarker = this.numberline?.leftMarker;
        const rightMarker = this.numberline?.rightMarker;
        ...
        this.createFractionStep({
            ...
            elements: [
                { type: 'text', value: i18n.t('tutorial.step4.1') },
                { type: 'expression', value: this.question.questionPrompt },
                { type: 'text', value: i18n.t('tutorial.step4.2') },
            ],
        });
    }

    private async playFractionStep6() {
        ...
        this.pointingHand = this.scene.addImage(targetPointX - 3, pointingHandY, 'pointing_hand').setRotation(Math.PI);
    ...
    private playFractionStep(step: number) {
        ...
        switch (step) {
            ...
            case 2:
                ...
                break;
            case 3:
                this.playFractionStep3();
                break;
            case 4:
                this.playFractionStep4();
                break;
            case 5:
                this.playFractionStep5();
                break;
            case 6:
                this.playFractionStep6();
                break;
        }
    }
