...
export interface MultiplicationInfoData {
    scene: BaseScene;
...
export class MultiplicationInfo {
    ...
    constructor(data: MultiplicationInfoData) {
    ...
    private playStep2() {
        ...
        // Show the actual jumps backwards on the number line
        ...
        const zeroMarkerIndex = this.question.markersList.indexOf('0' as never);
        const zeroPositionX = startX + width * zeroMarkerIndex;
        const jumpDir = operand1Index > zeroMarkerIndex ? 1 : -1;

        // Create yellow rectangles for each jump
        for (let jump = 0; jump < parseInt(operand2); jump++) {
            const jumpStartX = zeroPositionX + jumpDir * width * (jump + 1);
        ...
        this.createMultiplicationStep({
            audioKey: this.getAudioKey('step_2'),
            elements: [
                { type: 'text', value: i18n.t('howToPlay.multiplication.step2.1') },
                { type: 'expression', value: '0' },
                { type: 'text', value: i18n.t('howToPlay.multiplication.step2.2') },
                { type: 'expression', value: operand2 },
                { type: 'text', value: i18n.t('howToPlay.multiplication.step2.3') },
                { type: 'expression', value: operand1.replace('-', '') + '.' },
            ],
        });
    }
    ...
    private playStep3() {
        ...
        this.createMultiplicationStep({
            ...
            elements: [
                { type: 'text', value: i18n.t('howToPlay.multiplication.step3.1') },
                { type: 'expression', value: this.question.questionPrompt },
                { type: 'text', value: i18n.t('howToPlay.multiplication.step3.2') },
                { type: 'expression', value: formatMathExpression(this.question.shipLocation.toString()) + '.' },
            ],
        });
    }
    ...
    private playMultiplicationStep(step: number) {
    ...
    private createMultiplicationStep({
        audioKey,
        elements,
    }: {
        audioKey: string;
        ...
        stepAudio?.on('complete', () => {
            this.currentStep++;
            this.playMultiplicationStep(this.currentStep);
        });
    }
