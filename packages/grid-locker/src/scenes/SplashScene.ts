...
export class SplashScene extends BaseScene {
    ...
    init() {
        ...
        const logo = this.addImage(this.display.width / 2, 267.5, 'splashscreen_gametitle')
            .setOrigin(0.5);
    ...
    create() {
        ...
        const topic = gameConfigManager.get('topic') || 'g6_t2_integers_and_coordinate';

        AnalyticsHelper.createInstance('grid_lock', topic);
    }
}