...
export function setSceneBackground(imageUrl: string, applyTint: boolean = false) {
    ...
    // Set the background image with optional tint
    const finalBackgroundValue = applyTint
        ? `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), ${backgroundValue}`
        : backgroundValue;

    document.body.style.setProperty(`--scene-bg-${inactiveBuffer}`, finalBackgroundValue);
